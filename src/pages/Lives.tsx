...
export default function Lives() {
  ...
  const topPerformers = useMemo(() => {
    if (presenterMetrics.length === 0)
      return { lives: '', sales: '', revenue: '', conversion: '', retention: '' }
      ...
      >
        ...
        <section className="apple-card p-5 mb-8 flex flex-wrap items-center gap-8">
            ...
            <Popover open={datePickerOpen} onOpenChange={setDatePickerOpen}>
              <PopoverTrigger asChild>
                <button
                  ...
                  style={{ backgroundColor: 'rgba(0,0,0,0.03)', borderColor: '#E5E5E7' }}
                >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="px-4 py-2 rounded-xl text-[13px] font-semibold" style={{ backgroundColor: 'rgba(0, 113, 227, 0.08)', color: '#0071E3' }}>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="apple-card p-6">
            ...
            <h3 className="text-[28px] font-bold tracking-tight mt-1" style={{ color: '#1D1D1F' }}>
        ...
        <section className="apple-card p-8 mb-8">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h3 className="text-[20px] font-semibold tracking-tight" style={{ color: '#1D1D1F' }}>
        ...
        <section className="apple-card overflow-hidden mb-8">
          <div className="px-8 pt-8 pb-4">
            <h3 className="text-[18px] font-semibold tracking-tight mb-6" style={{ color: '#1D1D1F' }}>
              ...
              <table className="w-full text-left zebra-table">
                <thead>
                  <tr
                    ...
                    style={{ color: '#86868B', borderColor: '#E5E5E7', backgroundColor: 'rgba(251,251,253,0.5)' }}
                  >
                ...
                <tbody className="divide-y" style={{ borderColor: '#E5E5E7' }}>
                  {loading ? (
                    ...
                    presenterMetrics.map((row) => (
                      <tr key={row.presenter} className="hover:bg-black/[0.01] transition-colors" style={{ borderColor: '#E5E5E7' }}>
                        <td className="px-8 py-4">
                          <div className="flex items-center gap-3">
                            <div
                              className="size-9 rounded-full bg-center bg-cover border flex items-center justify-center"
                              style={{ borderColor: '#E5E5E7', backgroundColor: '#E5E7EB', color: '#86868B' }}
                            >
                        ...
                        <td className={`px-6 py-4 text-[14px] ${topPerformers.sales === row.presenter ? 'top-performer-bg' : ''}`} style={{ color: '#1D1D1F' }}>
                          <span className={topPerformers.sales === row.presenter ? 'top-performer-cell' : ''}>{row.sales}</span>
                        </td>
                        <td className={`px-6 py-4 text-[14px] ${topPerformers.revenue === row.presenter ? 'top-performer-bg' : ''}`} style={{ color: '#1D1D1F' }}>
                          <span className={topPerformers.revenue === row.presenter ? 'top-performer-cell' : ''}>{formatFullCurrency(row.revenue)}</span>
                        </td>
                        <td className={`px-6 py-4 text-[14px] ${topPerformers.conversion === row.presenter ? 'top-performer-bg' : ''}`} style={{ color: '#1D1D1F' }}>
                          <span className={topPerformers.conversion === row.presenter ? 'top-performer-cell' : ''}>{row.conversion.toFixed(1)}%</span>
                        </td>
                        <td className={`px-8 py-4 text-[14px] text-right ${topPerformers.retention === row.presenter ? 'top-performer-bg' : ''}`} style={{ color: '#1D1D1F' }}>
                          <span className={topPerformers.retention === row.presenter ? 'top-performer-cell' : ''}>{row.retention.toFixed(0)}%</span>
                        </td>
                      </tr>
                    ))
                  )}
              ...
              <table className="w-full text-left zebra-table">
                ...
                <tbody className="divide-y" style={{ borderColor: '#E5E5E7' }}>
                  {weekdayMetrics.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-8 py-12 text-center" style={{ color: '#86868B' }}>
                        Nenhum dado disponível
                      </td>
                    </tr>
                  ) : (
                    weekdayMetrics.map((row, idx) => (
                      <tr key={row.dayValue} className="hover:bg-black/[0.01] transition-colors" style={{ borderColor: '#E5E5E7' }}>
                        <td className="px-8 py-4">
                          <div className="flex items-center gap-3">
                            <div
                              className="size-9 rounded-full flex items-center justify-center"
                              style={{ backgroundColor: idx === 0 ? '#ECFDF5' : '#F5F5F7', color: idx === 0 ? '#34C759' : '#86868B' }}
                            >
                            ...
                            {idx === 0 && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ backgroundColor: '#ECFDF5', color: '#34C759' }}>
                                Melhor dia
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-[14px]" style={{ color: '#1D1D1F' }}>
                          {row.lives}
                        </td>
                        <td className="px-6 py-4 text-[14px]" style={{ color: '#1D1D1F' }}>
                          {row.sales}
                        ...
                        <td className="px-8 py-4 text-[14px] text-right" style={{ color: '#1D1D1F' }}>
          ...
          {analysisTab === 'period' && (
            <div className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 rounded-2xl" style={{ backgroundColor: '#F5F5F7' }}>
                  <p className="text-[12px] font-semibold uppercase tracking-wider mb-2" style={{ color: '#86868B' }}>
                    Período Selecionado
                  </p>
                ...
                <div className="p-6 rounded-2xl" style={{ backgroundColor: '#F5F5F7' }}>
                  ...
                  <p className="text-[28px] font-bold" style={{ color: '#1D1D1F' }}>
                    {filteredData.length}
                  </p>
                  <p className="text-[13px] mt-1" style={{ color: '#86868B' }}>
                    {(filteredData.length / (differenceInDays(dateRange.to, dateRange.from) + 1)).toFixed(1)} lives/dia
                  </p>
                </div>
                <div className="p-6 rounded-2xl" style={{ backgroundColor: '#F5F5F7' }}>
                  <p className="text-[12px] font-semibold uppercase tracking-wider mb-2" style={{ color: '#86868B' }}>
                  ...
                  <p className="text-[28px] font-bold" style={{ color: '#34C759' }}>
                    {filteredData.length > 0 ? formatFullCurrency(kpis.revenue.value / filteredData.length) : 'R$ 0,00'}
                  </p>
                  <p className="text-[13px] mt-1" style={{ color: '#86868B' }}>
        ...
        <section className="apple-card overflow-hidden">
          <div className="px-8 pt-8 pb-4">
            <h3 className="text-[18px] font-semibold tracking-tight mb-2" style={{ color: '#1D1D1F' }}>
            ...
            <table className="w-full text-left zebra-table">
              ...
              <tbody className="divide-y" style={{ borderColor: '#E5E5E7' }}>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={10} className="px-8 py-12 text-center" style={{ color: '#86868B' }}>
                      Nenhuma live encontrada para o período selecionado
                    </td>
                  </tr>
                ) : (
                  [...filteredData].reverse().map((live, idx) => (
                    <tr key={`${live.date}-${idx}`} className="hover:bg-black/[0.01] transition-colors" style={{ borderColor: '#E5E5E7' }}>
                      <td className="px-6 py-3 text-[13px] font-medium" style={{ color: '#1D1D1F' }}>
                        {format(parseISO(live.date), 'dd/MM/yyyy')}
                      </td>
                      <td className="px-4 py-3 text-[13px]" style={{ color: '#86868B' }}>
                        {live.weekday}
                      </td>
                      ...
                      <td className="px-4 py-3 text-[13px] text-right" style={{ color: '#1D1D1F' }}>
                        {live.peakViewers.toLocaleString('pt-BR')}
                      </td>
                      <td className="px-4 py-3 text-[13px] text-right" style={{ color: '#1D1D1F' }}>
                        {live.retainedViewers.toLocaleString('pt-BR')}
                      </td>
                      <td className="px-4 py-3 text-[13px] text-right font-semibold" style={{ color: '#1D1D1F' }}>
                        {live.sales}
                      </td>
                      <td className="px-4 py-3 text-[13px] text-right" style={{ color: live.conversionRate >= 3 ? '#34C759' : '#FF9500' }}>
                        {live.conversionRate.toFixed(1)}%
                      </td>
                      <td className="px-4 py-3 text-[13px] text-right" style={{ color: live.retentionRate >= 60 ? '#34C759' : '#FF9500' }}>
                        {live.retentionRate.toFixed(1)}%
                      </td>
                      <td className="px-4 py-3 text-[13px] text-right font-semibold" style={{ color: '#34C759' }}>
                        {formatFullCurrency(live.revenue)}
                      </td>
                      <td className="px-6 py-3 text-[13px] text-right" style={{ color: '#1D1D1F' }}>
