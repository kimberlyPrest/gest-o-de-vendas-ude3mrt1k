...
export function HostPerformanceChart({ data, loading }: HostPerformanceChartProps) {
    const chartData = useMemo(() => {
        const grouped = data.reduce<Record<string, { presenter: string; totalConversion: number; count: number; totalRevenue: number }>>((acc, curr) => {
                ...
                <div className="rounded-lg border bg-white p-3 shadow-md text-sm">
                    ...
                    <div className="space-y-1">
                        <p className="flex justify-between gap-4">
                            ...
                            <span className="font-bold text-yellow-600">{(data.avgConversion).toFixed(1)}%</span>
                        </p>
                        <p className="flex justify-between gap-4">
                            <span className="text-gray-500">Faturamento Total:</span>
                            <span className="font-medium">R$ {data.totalRevenue.toLocaleString('pt-BR')}</span>
                        </p>
                        <p className="flex justify-between gap-4">
    ...
    return (
        <div className="h-[350px] w-full p-4 bg-white rounded-xl border shadow-sm">
            <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-lg font-semibold text-gray-800">Ranking de Conversão</h3>
            </div>
